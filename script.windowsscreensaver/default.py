    ...
    def run(self):
        ...
        while True: 
            ...
            if svrcheck.returncode == 1:
                self.loop = False
                print '3 Screensaver: External Screensaver deactivated'
                self.exit_callback()
            if not self.loop:
                break
...
if __name__ == '__main__':
    ...
    if svrcheck.returncode == 0:
        ...
        del screensaver_gui
        #force disable of XBMC internal screensaver
        subprocess.Popen([cmd,'-k'])
    else:
        print '2-6 Screensaver: No Windows screensaver is set. XBMC saver enabled.'
